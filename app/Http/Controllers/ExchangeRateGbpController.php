...
use App\Models\ExchangeRateGbp;
use Illuminate\Http\JsonResponse;
use Carbon\Carbon;

class ExchangeRateGbpController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        $existing = ExchangeRateGbp::whereDate('updated_at', $date)->first();
        ...
        if ($existing) {
            ...
            return response()->json(['message' => 'GBP rate updated', 'data' => $existing]);
        }

        $newRate = ExchangeRateGbp::create([
            'currency_from' => 'GBP',
            'currency_to' => 'LKR',
            'rate' => $validated['rate'],
            'updated_at' => $date,
        ...
        return response()->json(['message' => 'GBP rate created', 'data' => $newRate]);
    }
    ...
    public function getLast7Days(Request $request): JsonResponse
    {
        ...
        $rates = ExchangeRateGbp::whereBetween('updated_at', [$startDate, $endDate])
            ->orderBy('updated_at')
            ->get();
    ...
    public function getByDate(Request $request): JsonResponse
    {
        ...
        $rate = ExchangeRateGbp::whereDate('updated_at', $targetDate)->first();
